...
class FooEvents_Ticket_Helper {
        ...
        if (is_object($post) && isset( $_POST )) {
            ...
            if( $post->post_type == "event_magic_tickets" && !empty($_POST['add_ticket']) ) {
                ...
                if(!empty($userID) && !empty($_POST['add_ticket'])) {
                    ...
                    $product = get_post($_POST['WooCommerceEventsEvent']);
    ...
    public function resend_ticket($postID) {
        ...
        $mailStatus = $this->MailHelper->send_ticket($to, $subject, $header.$body.$footer, $attachment);
    ...
    public function get_ticket_data($ticketID) {
        ...
        $ticket['price'] = get_post_meta( $ticket['WooCommerceEventsVariationID'], '_regular_price', true);

        if(empty($ticket['price'])) {

            $ticket['price'] = get_post_meta( $ticket['WooCommerceEventsVariationID'], '_sale_price', true);
        ...
        if(!empty($ticket['price'])) {
            ...
            $ticket['price'] = $currencySymbol.''.$ticket['price'];

        } else {

            $_product   = wc_get_product($ticket['WooCommerceEventsProductID']);
            $ticket['price']  = $_product->get_price_html();
